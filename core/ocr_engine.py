...
def extract_text_with_gemini(images: list) -> str:
    ...
    try:
        ...
        # Fallback cascade: Use correct 2026 Gemini models (cascade to avoid daily quota limits)
...
def segment_answers_with_gemini(raw_text: str, questions_list: list) -> dict:
    ...
    try:
        ...
        prompt = (
            f"You are an expert exam evaluator. You are given the raw OCR text of a student's answer sheet.\n"
            f"Your task is to accurately segment the raw text into distinct answers based on the provided Question IDs and topics.\n\n"
            f"Here are the exact Question IDs and their topics:\n{q_context}\n"
            f"CRITICAL RULES:\n"
            f"1. Match by context, not just numbers: Students often mislabel question numbers (e.g., writing '2a' instead of '2'). Read the content to map it correctly.\n"
            f"2. DO NOT DUPLICATE TEXT: A block of text can only belong to ONE question. Never assign the same text to multiple questions.\n"
            f"3. UNATTEMPTED QUESTIONS: If the student did not answer a specific question from the list above, you MUST set its value to an empty string (\"\"). Do not guess or force an answer.\n"
            f"4. ONLY use the text provided in the RAW OCR TEXT below. Do not generate or hallucinate any outside knowledge.\n\n"
